...
- 配置项可以按模块分组，例如 `site`、`profile`、`topNav`、`search`、`blog`、`home`。
...
搜索配置位于 `config.search`，控制站内搜索入口是否启用、快捷键提示、输入框占位文本，以及最多展示多少条结果：
...
普通页面、布局、RSS endpoint 也使用同一个 helper。这样站点标题、RSS 描述、首页数据、作者卡片、顶部导航、搜索行为和博客分页都来自同一份 TOML。
